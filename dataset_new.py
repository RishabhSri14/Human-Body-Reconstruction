...
class NeRF_DATA_NEW(Dataset):
    def __init__(self,
        ...
        filename = self.path[:self.path.rfind('/')]+self.dataset[0]['file_path'][self.dataset[0]['file_path'].find('.')+1:]

        self.H,self.W=self.data['h'],self.data['w']
        # print('\n\n', self.image_transforms, '\n')
        self.focal1=self.data['fl_x']
        self.focal2=self.data['fl_y']
    ...
    def __getitem__(self, idx):
        filename = self.path[:self.path.rfind('/')]+self.dataset[idx]['file_path'][self.dataset[idx]['file_path'].find('.')+1:]
        ...
        return image,torch.Tensor(self.dataset[idx]['transform_matrix']),self.dataset[idx]['sharpness']
